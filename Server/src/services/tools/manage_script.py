"""Script validation tool. For script edits, use script_apply_edits (structured operations)."""
...
def _split_uri(uri: str) -> tuple[str, str]:
    """Split a URI or path into (name, directory) under Assets/."""
    raw_path: str
...
@mcp_for_unity_tool(
    description="Validate a C# script and return diagnostics (warnings, errors). Unity auto-compiles on file changes, but this provides quick feedback without waiting.",
...
async def validate_script(
    ...
    uri: Annotated[
        str,
        "URI of the script to validate: mcpforunity://path/Assets/..., file://..., or Assets/...",
    ],
    level: Annotated[Literal["basic", "standard"], "Validation level"] = "basic",
    include_diagnostics: Annotated[
        bool, "Include full diagnostics and summary"
    ] = False,
) -> dict[str, Any]:
    ...
    if isinstance(resp, dict) and resp.get("success"):
        ...
        warnings = sum(
            1 for d in diags if str(d.get("severity", "")).lower() == "warning"
        )
        errors = sum(
            1 for d in diags if str(d.get("severity", "")).lower() in ("error", "fatal")
        )
