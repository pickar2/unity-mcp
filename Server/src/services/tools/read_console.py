...
@mcp_for_unity_tool(
    description="""Read or clear Unity console messages. Returns immediately with results.
...
async def read_console(
    ...
    format: Annotated[Literal['plain', 'detailed',
                              'json'], "Output format (ignored; entries are always structured with sequenceId/timestamp). Kept for backwards compatibility."] | None = None,
    ...
    # filter_text and filter_regex are mutually exclusive - choose one.
    ...
    if action == "get" and count is None and coerced_page_size is None and not count_only:
        count = 100
