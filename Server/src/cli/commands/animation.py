...
@click.option(
    "--layer", "-l", default=-1, type=int, help="Animator layer index (-1 for default)."
)
@click.option("--search-method", type=SEARCH_METHOD_CHOICE_BASIC, default=None)
@handle_unity_errors
def animator_play(
    target: str, state_name: str, layer: int, search_method: Optional[str]
):
...
@click.argument("state_name")
@click.option(
    "--duration", "-d", default=0.25, type=float, help="Crossfade duration in seconds."
)
@click.option(
    "--layer", "-l", default=-1, type=int, help="Animator layer index (-1 for default)."
)
...
@handle_unity_errors
def clip_create(
    clip_path: str, name: Optional[str], length: float, loop: bool, frame_rate: float
):
...
@click.option(
    "--type", "-t", "component_type", default="Transform", help="Component type name."
)
@click.option(
    "--keys",
    "-k",
...
@click.option(
    "--keys", "-k", required=True, help="Keyframes as JSON: [[0,0],[0.5,1],[1,0]]"
)
@handle_unity_errors
...
@handle_unity_errors
def clip_set_vector_curve(
    clip_path: str, vector_property: str, component_type: str, keys: str
):
...
@click.option(
    "--amplitude", "-a", default=1.0, type=float, help="Amplitude/intensity multiplier."
)
@click.option("--loop/--no-loop", default=True, help="Whether clip loops.")
@handle_unity_errors
def clip_create_preset(
    clip_path: str, preset: str, duration: float, amplitude: float, loop: bool
):
...
@click.argument("clip_path")
@click.option(
    "--function", "function_name", required=True, help="Function name to call."
)
...
@click.option(
    "--function", "function_name", default=None, help="Remove events by function name."
)
@click.option(
    "--time",
    type=float,
    default=None,
    help="Filter by time when removing by function name.",
)
@handle_unity_errors
...
@click.option(
    "--has-exit-time/--no-exit-time",
    default=True,
    help="Whether transition uses exit time.",
)
...
@handle_unity_errors
def controller_add_layer(
    controller_path: str, layer_name: str, weight: float, blending_mode: str
):
...
@click.option(
    "--blend-type",
    type=click.Choice(
        ["simpledirectional2d", "freeformdirectional2d", "freeformcartesian2d"]
    ),
...
@click.option(
    "--threshold", type=float, default=None, help="Threshold for 1D blend tree."
)
@click.option(
    "--position",
    type=(float, float),
    default=None,
    help="Position (x, y) for 2D blend tree.",
)
@click.option("--layer-index", type=int, default=0, help="Layer index.")
@handle_unity_errors
