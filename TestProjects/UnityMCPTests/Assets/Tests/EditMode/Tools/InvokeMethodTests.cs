...
namespace MCPForUnityTests.Editor.Tools
{
    public class InvokeMethodTests
    {
        ...
        [Test]
        public void InstanceMethod_ByRelativePath_ResolvesCorrectObject()
        {
            var parent = CreateTestObject("InvokeRelParent");
            var child = new GameObject("InvokeRelChild");
            testObjects.Add(child);
            child.transform.SetParent(parent.transform);
            child.transform.localPosition = Vector3.zero;
            ...
            {
                ["method"] = "Translate",
                ["target"] = "InvokeRelParent/InvokeRelChild",
                ["component"] = "Transform",
                ["args"] = new JArray(2f, 0f, 0f)
            }));

            Assert.IsTrue(result.Value<bool>("success"), result.ToString());
            Assert.AreEqual(2f, child.transform.localPosition.x, 0.01f);
        }
