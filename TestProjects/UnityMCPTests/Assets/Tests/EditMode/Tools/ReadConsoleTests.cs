...
namespace MCPForUnityTests.Editor.Tools
{
    public class ReadConsoleTests
    {
        ...
        [Test]
        public void HandleCommand_Get_WithFilterText_MatchesSubstring()
        {
            ...
            var result = ToJObject(ReadConsole.HandleCommand(new JObject
            {
                ["action"] = "get",
                ["types"] = new JArray { "log" },
                ["filterText"] = $"DIAGNOSTIC-{uniqueId}",
                ["count"] = 100
            }));
            ...
            var entries = result["data"]?["entries"] as JArray;
            Assert.IsNotNull(entries);
            Assert.AreEqual(1, entries.Count, "Should only match the DIAGNOSTIC line");
            Assert.That(entries[0]["message"]?.ToString() ?? "", Does.Contain($"DIAGNOSTIC-{uniqueId}"));
        }
        ...
        public void HandleCommand_Get_WithRegexFilter_MatchesPattern()
        {
            ...
            var result = ToJObject(ReadConsole.HandleCommand(new JObject
            {
                ["action"] = "get",
                ["types"] = new JArray { "log" },
                ["filterRegex"] = $"DIAGNOSTIC-{uniqueId}|CONTACTS-{uniqueId}.*particle 1842",
                ["count"] = 100
            }));
            ...
            foreach (var entry in entries)
            {
                ...
                {
                    if (msg.Contains($"DIAGNOSTIC-{uniqueId}") || msg.Contains($"CONTACTS-{uniqueId}"))
                        matchCount++;
                    if (msg.Contains($"UNRELATED-{uniqueId}"))
                        foundUnrelated = true;
                }
            }
