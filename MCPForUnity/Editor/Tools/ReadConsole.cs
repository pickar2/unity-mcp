...
namespace MCPForUnity.Editor.Tools
{
    ...
    /// Reads from <see cref="LogCaptureService"/>'s in-memory ring buffer instead of
    /// reflecting into Unity's internal LogEntries on every call. Each entry carries
    /// a sequenceId and timestamp so callers can poll efficiently via sinceSequenceId.
    /// </summary>
    ...
    public static class ReadConsole
    {
        ...
        static ReadConsole()
        {
            ...
            {
                McpLog.Error(
                    $"[ReadConsole] Failed to reflect LogEntries.Clear: {e.Message}"
                );
            }
        }
        ...
        {
            // Clear the in-memory capture buffer first so concurrent readers see an
            // empty snapshot even if the Unity UI clear lags behind.
        ...
        /// <summary>
        /// Retrieves console log entries from the LogCaptureService ring buffer with
        /// optional filtering (text/regex), paging, and since-sequence/timestamp polling.
        /// </summary>
        private static object GetConsoleEntries(ToolParams p)
        {
            ...
            // filterText and filterRegex are mutually exclusive - choose one.
            ...
            if (usePaging)
            {
                int resolvedPageSize = Mathf.Clamp(pageSize ?? 50, 1, 5000);
