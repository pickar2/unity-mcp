    ...
    public static class SceneObject
    {
        ...
        private static List<object> SerializeComponents(GameObject go, HashSet<string> componentFilters = null, HashSet<string> propertiesFilter = null, bool includeInternal = true)
        {
            ...
            foreach (var comp in go.GetComponents<Component>())
            {
                ...
                // NOTE: Upstream's GameObjectSerializer.GetComponentData exposes includeNonPublicSerializedFields
                // rather than beta's separate includeInternal axis. We map includeInternal to that parameter so
                // the get action still throttles verbosity when the agent does not ask for everything.
